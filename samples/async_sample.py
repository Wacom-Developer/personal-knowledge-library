...
def import_entity_from_wikidata(search_term: str, locale: LanguageCode) -> Dict[str, ThingObject]:
    ...
    for res in qid_entities:
        wikidata_thing, _ = wikidata_to_thing(
            res,
...
async def user_management_sample(tenant_api_key: str, instance: str) -> Tuple[User, str, str]:
    ...
    user_management: AsyncUserManagementService = AsyncUserManagementService(
        application_name="Async user management sample", service_url=instance
    )
    meta_data: dict = {"user-type": "demo"}
    user, user_token, refresh_token, _ = await user_management.create_user(
        tenant_key=tenant_api_key, external_id=uuid.uuid4().hex, meta_data=meta_data, roles=[UserRole.USER]
    )
    ...
    for user in users:
        if "user-type" in user.meta_data and user.meta_data["user-type"] == "demo":
            await user_management.delete_user(
                tenant_key=tenant_api_key, external_id=user.external_user_id, internal_id=user.id, force=True
            )
...
async def main(external_user_id: str, tenant_api_key: str, instance: str):
    ...
    print(f"User: {user_2}")
    print("-" * 100)
    async_client_user_1: AsyncWacomKnowledgeService = AsyncWacomKnowledgeService(
        application_name="Async user 1", service_url=instance
    )
    refresh_session_1: RefreshableSession = await async_client_user_1.register_token(
        auth_key=user_token_1, refresh_token=refresh_token_1
    )
    async_client_user_2: AsyncWacomKnowledgeService = AsyncWacomKnowledgeService(
        application_name="Async sample", service_url=instance
    )
    ...
    group_management: AsyncGroupManagementService = AsyncGroupManagementService(
        application_name="Group management", service_url=instance
    )
    ...
    print(
        f"User 2 can see the entities now. Let us check with async client 2. "
    ...
    for thing in created:
        ...
        print(
            f'User 2 can see entity {label.content if label else "UNKNOWN"} {iter_thing.uri}.'
