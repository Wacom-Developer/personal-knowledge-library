...
class SessionManagerTest(TestCase):
    ...
    def test_2_session_manager(self):
        ...
        knowledge_service: WacomKnowledgeService = WacomKnowledgeService(
            application_name="Session manager 1", service_url=os.environ.get("INSTANCE")
        )
        # Token manager
        ...
        self.assertNotEqual(id(token_manager_1), id(token_manager_2), "The token manager is not a singleton, anymore.")
        user_token, refresh_token, _ = knowledge_service.request_user_token(
            self.tenant_api_key, self.cache.external_id_1
        )
        # Add user session
        session: PermanentSession = token_manager_1.add_session(
            user_token, refresh_token, self.tenant_api_key, self.cache.external_id_1
        )
        ...
        self.assertLessEqual(session.expires_in, 3600.0)
        # The token is valid for 1 hour (3600 seconds)
        self.assertGreaterEqual(
            session.service_url, os.environ.get("INSTANCE", "").replace("https://", "").replace("http://", "")
        )

    def test_3_login(self):
        knowledge_service_1: WacomKnowledgeService = WacomKnowledgeService(
            application_name="Session manager 1", service_url=os.environ.get("INSTANCE")
        )
        knowledge_service_2: WacomKnowledgeService = WacomKnowledgeService(
            application_name="Session manager 1", service_url=os.environ.get("INSTANCE")
        )
    ...
    def test_4_duplicate(self):
        ...
        self.assertNotEqual(
            self.cache.external_id_1, self.cache.external_id_2, "The external ids are equal, but they should not be."
        )
        # Check if user is created
    ...
    def teardown_class(cls):
        """Clean up the user."""
        list_user_all: List[User] = cls.user_management.listing_users(
            cls.tenant_api_key, limit=SessionManagerTest.LIMIT
        )
        for u_i in list_user_all:
            if "account-type" in u_i.meta_data and u_i.meta_data.get("account-type") == "qa-test":
                ...
                cls.user_management.delete_user(
                    cls.tenant_api_key, external_id=u_i.external_user_id, internal_id=u_i.id, force=True
                )
